import type { WordleLevel } from "@/features/puzzles";
import type { LocalPuzzleAttempt } from "./puzzle-attempt-history";

export type WordleLevelState = {
  level: WordleLevel;
  locked: boolean;
  completed: boolean;
};

export type WordleHubProgress = {
  dailyLevel: WordleLevel | null;
  levels: WordleLevel[];
  levelStates: WordleLevelState[];
  currentStreak: number;
  maxStreak: number;
};

export function buildWordleHubProgress({
  levels,
  attempts,
  today = new Date(),
}: {
  ...
  const completedLevelIds = new Set(attempts.filter((attempt) => attempt.puzzleType === "wordle").map((attempt) => attempt.levelId));
...
function countContiguousCompletions(levels: WordleLevel[], completedLevelIds: Set<string>) {
  let count = 0;
...
function completionDays(attempts: LocalPuzzleAttempt[]) {
  return new Set(
    attempts
      .filter((attempt) => attempt.puzzleType === "wordle")
